...
const reiniciarMarcador = document.getElementById("reiniciarMarcador")
const anuncioGanador = document.getElementById("anuncioGanador")
...
let victoriasO = localStorage.getItem("victoriasO")
iA.innerText = localStorage.getItem("victoriasO")
let cantMovs = 0


jugador.textContent = 0
iA.textContent = 0
    ...
    lista.forEach((item) => item.addEventListener("click", function () {      //cuando entra al if inserta una X cuando hace clic en una posicion de la lista, despues evalua ejecuta la funcion de la supuesta IA que coloca un circulo y finalmente evalua quien gano
        if (item.innerHTML === "" && !elegirGanador()) {                 //crear una variable para guardar la lista con un filtro de las posiciones vacias
           if(!hayGanador){
            ...
            item.style.color = "blue"; // Cambiar el color de la X a azul
            elegirGanador()
...
function marcaCirculo() {                                                              //crear una variable para guardar la lista con un filtro de las posiciones vacias
    ...
    casillasVacias[posicionAleatoria].style.color = "red"
    elegirGanador()
}
    ...
    for (const iterar of posicionesGanadoras) {                              //creo un for of para iterar el arreglo y una variable para guardar las iteraciones
        ...
        if (lista[pos1].textContent != "" &&                                 // espacio vacio y despues comparo si la primera posicion es igual a las demás y si eso se cumple que muestre una
            ...
            if (lista[pos1].textContent === "X") {
                ...
                console.log(jugador);
                let ganadorAnunciado = document.createElement("h2")
                ganadorAnunciado.innerHTML = "Felicidades has ganado"
                anuncioGanador.appendChild(ganadorAnunciado)
                return
                ...
                let ganadorAnunciado = document.createElement("h2")
                ganadorAnunciado.innerHTML = "Que malo has perdido contra la maquina"
                anuncioGanador.appendChild(ganadorAnunciado)
                    return
            }
            ...
            console.log("Empate");
            let ganadorAnunciado = document.createElement("h2")
            ganadorAnunciado.innerHTML = "Empate =("
            anuncioGanador.appendChild(ganadorAnunciado)
            return
        }
        hayGanador = false
...
function reinicio() {
    btnReiniciar.addEventListener("click", function () {
        lista.forEach(item => {
            item.innerHTML = ""
            anuncioGanador.innerHTML = ""
            cantMovs = 0
        });
    })
}

function btnReinicioMarcador() {
    reiniciarMarcador.addEventListener("click", function () {
        localStorage.clear()
        jugador.innerHTML = ""
        iA.innerHTML = ""
        location.reload()
